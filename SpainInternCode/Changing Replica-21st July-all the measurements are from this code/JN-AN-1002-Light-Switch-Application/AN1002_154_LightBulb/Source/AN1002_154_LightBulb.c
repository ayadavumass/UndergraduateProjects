...
PRIVATE uint16 Array_HashX[MAX_REPLICA];
PRIVATE uint16 Array_HashY[MAX_REPLICA];
...
PRIVATE char Str[50];
PRIVATE int EPOCH_No=-1;
PRIVATE int Traffic_Measure[MAX_REPLICA][2];
PRIVATE tsReplication_Stat Replication_Stat;
PRIVATE tsReplication_Stat SendingReplication_Stat;
PRIVATE tsNextEpochInfo NextEpochInfo;
PRIVATE int Traffic_Counter=0;

PRIVATE int No_Stat_Recv=0;
PRIVATE int Curr_Replicas=NO_REPLICA;
PRIVATE int Epoch_Expiry_Time=Run_Time*60;
PRIVATE int No_Replica_NextEpoch=NO_REPLICA;

PRIVATE int Wakeup_Curr_Time=0;
PRIVATE bool NextEpoch_Info=FALSE;
PRIVATE int PreCurr_Replicas;
PRIVATE uint16 PreReplica_Num;
PRIVATE uint16 PreArray_HashX[MAX_REPLICA];
PRIVATE uint16 PreArray_HashY[MAX_REPLICA];
PRIVATE uint8 Num_Replicas_Node=0;
PRIVATE uint8 Replica_Num_Array[MAX_REPLICA];
PRIVATE int ACK_Timer=0;
PRIVATE bool ACK_Wait=FALSE;
PRIVATE int No_ACK_Recv=0;
PRIVATE int ACK_Recv[MAX_REPLICA];
PRIVATE int ACK_Retry=0;
PRIVATE int NextEpochNo=7;
PRIVATE bool Measure_Flag=TRUE;
...
PRIVATE void Replication_Algo(uint8 Rep_Num,uint8 Replica_Num_Used);
...
PRIVATE void EpochExpiry();
PRIVATE void Epoch_Initialization();
PRIVATE void Send_Rep_Stat();
PRIVATE void Replication_Tree_RepStat(uint8 Get_Root_Node,uint8 No_Of_Replica);
...
PRIVATE bool Replica_Num_Check(uint16 DstX,uint16 DstY);
...
PRIVATE bool Replica_Num_Check(uint16 DstX,uint16 DstY)
{
    ...
    bool Me_Flag=FALSE;
                     ...
                     {
                        Flag=FALSE;
                        if(Array_HashX[i]==DstX&&Array_HashY[i]==DstY)
                         {
                             Me_Flag=TRUE;

                             for(j=0;j<Num_Replicas_Node;j++)
                             {
                                 if(Replica_Num_Array[j]==i)
                                 {
                                     Flag=TRUE;
                                     break;
                            ...
                            if(Flag==FALSE)
                            {
                                ...
                                Replica_Num_Array[Num_Replicas_Node]=i;
                                Num_Replicas_Node++;
                                break;
...
/*PRIVATE void Topology(uint16 ShorAddress)
{
    ...
    Node_Info[1].Address.Node_Shrt_Addr=0x0503U;
    Node_Info[1].G_Loc.X=95;
    Node_Info[1].G_Loc.Y=1;

    Node_Info[2].Address.Node_Shrt_Addr=0x0504U;
    Node_Info[2].G_Loc.X=190;
    Node_Info[2].G_Loc.Y=1;

    Node_Info[3].Address.Node_Shrt_Addr=0x0505U;
    Node_Info[3].G_Loc.X=1;
    Node_Info[3].G_Loc.Y=95;

    Node_Info[4].Address.Node_Shrt_Addr=0x0506U;
    Node_Info[4].G_Loc.X=95;
    Node_Info[4].G_Loc.Y=95;

    Node_Info[5].Address.Node_Shrt_Addr=0x0507U;
    Node_Info[5].G_Loc.X=190;
    Node_Info[5].G_Loc.Y=95;

    Node_Info[6].Address.Node_Shrt_Addr=0x0508U;
    Node_Info[6].G_Loc.X=1;
    Node_Info[6].G_Loc.Y=190;

    Node_Info[7].Address.Node_Shrt_Addr=0x0509U;
    Node_Info[7].G_Loc.X=95;
    Node_Info[7].G_Loc.Y=190;

    Node_Info[8].Address.Node_Shrt_Addr=0x0510U;
    Node_Info[8].G_Loc.X=190;
    Node_Info[8].G_Loc.Y=190;
    ...
    if(OPR_MODE==0&&START)
            {
              ...
              /*   vPrintString("Wakeup_Curr_Time ");
                 vDisplayHex16(Wakeup_Curr_Time);
                  ...
                  if(Traffic_Counter==(Traffic_Measurement_Time*60))
            {
                Measure_Flag=FALSE;
            ...
            if((No_Stat_Recv==Curr_Replicas)&&NextEpoch_Info==FALSE&&ACK_Wait==FALSE&&(No_ACK_Recv==Curr_Replicas-Num_Replicas_Node))
                {
                   ...
                   // No_Replica_NextEpoch=var;
                    No_Replica_NextEpoch=1;

                    NextEpoch_Info=TRUE;
                    NextEpochNo=EPOCH_No+1;
                ...
                if(Wakeup_Curr_Time==Epoch_Expiry_Time&&ACK_Wait==FALSE&&NextEpoch_Info==TRUE)
                {
...
PRIVATE void EpochExpiry()
{
    ...
    vPrintString("In Func Current Epoch Expires \r\n");
                 PreCurr_Replicas=Curr_Replicas;
                 PreReplica_Num=Replica_Num;
                 int i=0;
                 for(i=0;i<Curr_Replicas;i++)
                 {
                     PreArray_HashX[i]=Array_HashX[i];
                     PreArray_HashY[i]=Array_HashY[i];
                 }
                 ...
                 if(Node_Type==REPLICA)
                 {
                    Temp_Replica=FALSE;
                    Node_Type=NOTHING;
                    Perform_Replica_Transtion(PreReplica_Num);
...
PRIVATE void Epoch_Initialization()
{
    ...
    Curr_Replicas=No_Replica_NextEpoch;
    EPOCH_No=NextEpochNo;
    vPrintString("EPoch Initialisation \r\n");
    int i=0;
    for(i=0;i<MAX_REPLICA;i++)
    {
        Traffic_Measure[i][0]=-1;
        Traffic_Measure[i][1]=-1;
        ACK_Recv[i]=-1;
    }

    Replication_Stat.Tr_Mesg_Cons=0;
    Replication_Stat.Tr_Mesg_Prod=0;
    No_Stat_Recv=0;
    NextEpoch_Info=FALSE;
    Epoch_Expiry_Time=Run_Time*60;
    Traffic_Counter=0;

    Calculate_Hashes(Curr_Replicas);
      ...
      for (i=0;i<MAX_REPLICA;i++)
      {
       ...
       for(i=0;i<Curr_Replicas;i++)
    {
        uint16 NextHop_Address=Calculate_NextHopAddress(Source_X,Source_Y,Array_HashX[i],Array_HashY[i]);
        if(NextHop_Address==COORD_ADDR)
        {
            Replica_Num_Check(Array_HashX[i],Array_HashY[i]);
        }
    }
    vPrintString("Num_Replica_Node ");
    vDisplayHex16(Num_Replicas_Node);
...
PRIVATE void Send_Rep_Stat()
{
    vPrintString("SENDING REPLICA REPLICATION STATISTICS \r\n");
                ...
                vDisplayHex16(Replication_Stat.Tr_Mesg_Prod);
             //   vNum2StringPrint(Replication_Stat.Tr_Mesg_Prod,10);
                vPrintString("\tConsumption Mesg");
                vDisplayHex16(Replication_Stat.Tr_Mesg_Cons);
               // vNum2StringPrint(Replication_Stat.Tr_Mesg_Cons,10);

          if(Traffic_Measure[Replica_Num][0]==-1)
                {
                    No_Stat_Recv=Num_Replicas_Node+No_Stat_Recv;
                    Traffic_Measure[Replica_Num][0]=Replication_Stat.Tr_Mesg_Prod;
                    Traffic_Measure[Replica_Num][1]=Replication_Stat.Tr_Mesg_Cons;
...
//                    int a=Epoch_Expiry_Time-Wakeup_Curr_Time;
                ...
                SendingReplication_Stat.Tr_Mesg_Prod=Replication_Stat.Tr_Mesg_Prod;
                SendingReplication_Stat.Tr_Mesg_Cons=Replication_Stat.Tr_Mesg_Cons;



        Replication_Tree_RepStat(Replica_Num,Num_Replicas_Node);
               /*     int i=0;
    for(i=0;i<MAX_REPLICA;i++)
    {
        ACK_Recv[i]=-1;
    }*/
    if(Num_Replicas_Node==Curr_Replicas)
    {
        ACK_Wait=FALSE;
    }else{
        ACK_Wait=TRUE;
...
{
    int Count=0;
    for(Count=0;Count<Num_Replicas_Node;Count++)
    {
   ...
   Replication_Algo(Get_Root_Node,Replica_Num_Array[Count]);
   ...
   // Routing_Header.Dst_Info.DstY=Array_HashY[Num];       // 10-11
    ...
    DCS_Header.Epoch_No=EPOCH_No;

    uint8 * PtrD=(uint8 *)&SendingReplication_Stat;
...
void Perform_Replica_Transtion(uint16 PreReplica_Num)
{
    int i=0,j=0,index=-1;
    int dist,tempdist;
    ...
    for(i=0;i<Curr_Replicas;i++)
    {
        dist=2*((DIMX*DIMX)+(DIMY*DIMY));
        index=-1;
        for(j=0;j<PreCurr_Replicas;j++)
        {
            tempdist=(PreArray_HashX[j]-Array_HashX[i])*(PreArray_HashX[j]-Array_HashX[i])+(PreArray_HashY[j]-Array_HashY[i])*(PreArray_HashY[j]-Array_HashY[i]);
            if(tempdist<dist)
            {
                dist=tempdist;
                index=j;
        ...
        uint16 NextHop_Addr1=Calculate_NextHopAddress(Source_X,Source_Y,PreArray_HashX[index],PreArray_HashY[index]);
        if(NextHop_Addr1==COORD_ADDR)
       // if(index==PreReplica_Num)
        {
            vPrintString("\tSending Replica Update \r\n");
            uint16 DstX=Array_HashX[i];
            uint16 DstY=Array_HashY[i];
            uint16 NextHop_Addr=Calculate_NextHopAddress(Source_X,Source_Y,DstX,DstY);
    ...
    {
        Rep_Data[i]=TEMP_Storage[TEMP_Counter-i];
    }
...
PRIVATE void vProcessIncomingData(MAC_McpsDcfmInd_s *psMcpsInd)
{
    ...
    if (psMcpsInd->u8Type == MAC_MCPS_IND_DATA)
    {
       ...
       //  || (u16NodeAddr == sCoordData.sNode.asAssocNodes[1].u16ShortAddr)||u16NodeAddr==0x0503U)
        tsHeaders *PtrT=(tsHeaders *)psFrame->au8Sdu;
        if(valid)
        {
            Msg_Recv++;
    ...
    Ptr->Routing_Header.TTL=Ptr->Routing_Header.TTL-1;
        ...
        if(NextHop_Addr==Src_Add&&PtrT->DCS_Header.Epoch_No==EPOCH_No)
        {
            ...
            vDisplayHex16(Ptr->Routing_Header.Dst_Info.DstY);
             ...
             if(Ptr->DCS_Header.OpCode==PUT&&Ptr->DCS_Header.AppID==TEMP)
             {
                ...
                bool Flag=Replica_Num_Check(Ptr->Routing_Header.Dst_Info.DstX,Ptr->Routing_Header.Dst_Info.DstY);
           ...
           /*  vDisplayHex16(PtrD[0].Value);
             ...
             if(Temp_Replica==FALSE&&Num_Replicas_Node>0)
                {
                     Temp_Replica=TRUE;
                     Node_Type=REPLICA;
                     vLightBulbInit();
...
uint16 NextHop_Addr=Calculate_NextHopAddress(Source_X,Source_Y,SrcX,SrcY);
    ...
    if(Flags==1&&NextEpoch_Info==TRUE)
    {
        DCS_Header.Flags=1;      //13              Next Epoch Info Being Sent

        NextEpochInfo.EpochExpiryTime=Epoch_Expiry_Time-Wakeup_Curr_Time;
        NextEpochInfo.NextNoReplica=No_Replica_NextEpoch;
        NextEpochInfo.Num_NextEpoch=EPOCH_No+1;
...
uint16 NextHop_Addr=Calculate_NextHopAddress(Source_X,Source_Y,Ptr->Routing_Header.Src_Info.SrcX,Ptr->Routing_Header.Src_Info.SrcY);
    ...
    if(Ptr->DCS_Header.Flags==1&&NextEpoch_Info==TRUE)
    {
        DCS_Header.Flags=1;      //13              Next Epoch Info Being Sent
        ...
        vPrintString("\tI am the Destination,Data won't GET_RESPONSE \r\n");
            ...
            vPrintString("RECIEVED GET RESPONSE \r\n");
             ...
             for(i=0;i<Ptr->DCS_Header.Len;i++)
                 ...
                 if(Ptr->DCS_Header.Flags==1&&NextEpoch_Info==FALSE)
                 {
                     tsNextEpochInfo *NextEpochPtr=&ptr8[sizeof(Headers)+Ptr->DCS_Header.Len*sizeof(tsData_Storage)];
                     Epoch_Expiry_Time=Wakeup_Curr_Time+NextEpochPtr->EpochExpiryTime;
                     No_Replica_NextEpoch=NextEpochPtr->NextNoReplica;
                     NextEpoch_Info=TRUE;
                     NextEpochNo=NextEpochPtr->Num_NextEpoch;
                     vPrintString("\t Epoch_Expiry_Time");
                     vDisplayHex16(Epoch_Expiry_Time);
                   //  vNum2StringPrint(Epoch_Expiry_Time,10);
            ...
            }else if(Ptr->DCS_Header.OpCode==REPLICA_STATISTICS)
            {
              ...
              SendingReplication_Stat.Tr_Mesg_Prod=PtrD->Tr_Mesg_Prod;
              SendingReplication_Stat.Tr_Mesg_Cons=PtrD->Tr_Mesg_Cons;
              uint8 Root_Node=Ptr->DCS_Header.Rep_Num;
                if(Traffic_Measure[Root_Node][0]==-1)
                {
                    No_Stat_Recv=Ptr->DCS_Header.Flags+No_Stat_Recv;
                    Traffic_Measure[Root_Node][0]=PtrD->Tr_Mesg_Prod;
                    Traffic_Measure[Root_Node][1]=PtrD->Tr_Mesg_Cons;
                }
                /*if((No_Stat_Recv==Curr_Replicas)&&NextEpoch_Info==FALSE)
                {
  ...
  Replication_Tree_RepStat(Root_Node,Ptr->DCS_Header.Flags);
   ...
   uint16 NextHop_Addr=Calculate_NextHopAddress(Source_X,Source_Y,Array_HashX[Root_Node],Array_HashY[Root_Node]);
    ...
    Routing_Header.Src_Info.SrcX=Source_X;       //4-5
    Routing_Header.Src_Info.SrcY=Source_Y;       // 6-7
    Routing_Header.Dst_Info.DstX=Array_HashX[Root_Node];       // 8-9
    Routing_Header.Dst_Info.DstY=Array_HashY[Root_Node];       // 10-11

                vPrintString("SENDING ACK \r\n");

        DCS_Header.OpCode=ACK;     //12

        DCS_Header.Flags=Num_Replicas_Node;      //13
...
if(NextHop_Addr==Src_Add)
        {
           ...
           }else if(Ptr->DCS_Header.OpCode==ACK)
           {
               ...
               if(ACK_Wait==TRUE&&ACK_Recv[Ptr->DCS_Header.Rep_Num]==-1)
               {
                   vPrintString("Ptr->DCS_Header.Rep_Num");
                   vDisplayHex16(Ptr->DCS_Header.Rep_Num);
                   ACK_Recv[Ptr->DCS_Header.Rep_Num]=1;

                   No_ACK_Recv=No_ACK_Recv+Ptr->DCS_Header.Flags;
               }

               if(No_ACK_Recv==(Curr_Replicas-Num_Replicas_Node)&&ACK_Wait==TRUE)
               {
        ...
        else if((psAHI_Ind->u32ItemBitmap & E_AHI_DIO10_INT))
        {
           ...
           // Send_Rep_Stat();
             ...
             vPrintString(" No_Stat_Recv");
                  ...
                  vDisplayHex16(EPOCH_No);

                   vPrintString(" No ACK");
                  vDisplayHex16(No_ACK_Recv);
            ...
            }else
            {
                ...
                vPrintString("Wakeup_Curr_Time ");
                 vDisplayHex16(Wakeup_Curr_Time);
                  ...
                  vDisplayHex16(No_Replica_NextEpoch);
                  int i=0;
                  vPrintString("Traffic Meausure ");
                  for(i=0;i<Curr_Replicas;i++)
                  {
                     vNum2StringPrint(i);
                     vNum2StringPrint(Traffic_Measure[i][0]);
                     vNum2StringPrint(Traffic_Measure[i][1]);
                  }
   ...
   /* uint16 u16LightSensor,u16TempSensor,u16HumidSensor;
    ...
    vDisplayHex16(sizeof(Routing_Header));
     ...
     if(NextHop_Addr==Src_Add)
     {
        ...
        Closest_Replica(Source_X,Source_Y);
         bool Flag=Replica_Num_Check(Routing_Header.Dst_Info.DstX,Routing_Header.Dst_Info.DstY);
