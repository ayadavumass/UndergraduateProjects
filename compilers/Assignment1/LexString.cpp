...
Return StringAnalysis(char input,int *CurrState,bool* Flag)
{
	switch(*CurrState)
	{
		case 0:
...
void InputBuffering(DFANode * KeywordStartNode,DFANode * OperatorStartNode,list<SymbolTableEntry> * SymbolTable)
{
	...
	LexicalAnalysisType Type=String;
	int CurrState=0;
	...
	while(run)
	{
		...
		if(Buffer[forward]==EOF)
		{
			...
			if(StrFlag==true)
			{
				cout<<"ERROR: Unterminated String at EOF"<<endl;
			}
			run=false;
		}
		Return Value;
		if(Type==String)
		{
			Value=StringAnalysis(Buffer[forward],&StrCurrState,&StrFlag);
		}
		if(Type==Comment)
		{
		...
		}else if(Value==SUCCESS)
		{
			...
			if(Type!=WhiteSpaces&&Type!=Comment&&Type!=String)
			{
			...
			if(Type==Punctuators||Type==Comment||Type==String)    //in case of Punctors it doesn't see two character, so it doesn't advance automatically, Buffer can be reloaded if its at verge.
			{
				forward++;
				pos++;
			}
			...
			if((Type==Comment&&CommFlag==true)||(Type==String&&StrFlag==true))
			{
