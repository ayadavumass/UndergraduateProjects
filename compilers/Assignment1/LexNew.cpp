...
{
	int BufferPartition=BUFFER_SIZE-1;

	int pos=0;
	char Buffer[2*BUFFER_SIZE],ch;
	Buffer[BufferPartition]=EOF;
	Buffer[2*BUFFER_SIZE-1]=EOF;
	...
	/*	switch(forward)
		...
		if(BufferPartition==forward)
		{
						is.read(&Buffer[BufferPartition+1],2*BUFFER_SIZE-(BufferPartition+1));
						if(is.eof())
						{
							Buffer[BufferPartition+1+is.gcount()]=EOF;
						}
					
		}else if(2*BUFFER_SIZE-1==forward)
		{
						is.read(Buffer,BufferPartition+1);
						if(is.eof())
						{
							Buffer[is.gcount()]=EOF;
						}
