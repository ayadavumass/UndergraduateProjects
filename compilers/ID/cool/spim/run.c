...
/* $Header: /project/cs/aiken/aa/projects/SourceRepository/cool/cool/spim/run.c,v 1.4 2003/04/27 19:57:06 matth Exp $
*/
  ...
  for (step_size = MIN (next_step, steps_to_run);
      ...
      for (step = 0; step < step_size; step += 1)
	{
	  ...
	  switch (OPCODE (inst))
	    ...
	    case Y_BC2F_OP:
	    case Y_BC3F_OP:
              statistics_inc_branches(&global_stats);   /* COOL */
	    ...
	    case Y_LB_OP:
              statistics_inc_reads(&global_stats);   /* COOL */
	    ...
	    case Y_SW_OP:
	      statistics_inc_writes(&global_stats);   /* COOL */
	  ...
	  PC += BYTES_PER_WORD;
	  statistics_inc_instruction_count(&global_stats);   /* COOL */
