...
/* $Header: /project/cs/aiken/aa/projects/SourceRepository/cool/cool/spim/spim.c,v 1.4 2003/05/03 01:22:15 matth Exp $
*/
...
#include "scanner.h"
#include "y.tab.h"
#include "statistics.h"                    /* Added for COOL */
  ...
  if (argc == 2)
    {
      ...
      assembly_file_read |= !read_assembly_file (argv[1]); //COOL: bugfix 
    }
  else
    for (i = 1; i < argc; i++)
      {
	...
	if (streq (argv [i], "-bare"))
	  ...
	  quiet = 0;
	else if (streq (argv [i], "-file") && (i+1 < argc)) //COOL: bugfix 
	  {
	    argv_ptr = i + 1;
	...
	else if (streq (argv [i], "-keepstats"))         /* Added for COOL */
	  keep_stats = 1;                                /* Added for COOL */
	else
	  //	  error ("usage: spim -bare/-asm -trap/-notrap -trap_file <file> -quiet/-noquiet -mapped_io/-nomapped_io -file <file> <args>\n");
	  error ("usage: spim -bare/-asm -trap/-notrap -trap_file <file> -quiet/-noquiet -mapped_io/-nomapped_io -keepstats -file <file> <args>\n"); /* Changed for COOL */
      }
