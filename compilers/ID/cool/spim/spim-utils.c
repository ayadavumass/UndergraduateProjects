...
/* $Header: /project/cs/aiken/aa/projects/SourceRepository/cool/cool/spim/spim-utils.c,v 1.5 2003/04/27 19:45:06 matth Exp $
*/
...
#endif
{
  ...
  if (trap_file != NULL)
	...
	fatal_error ("Cannot read trap handler: %s\n", trap_file);
      ...
      if (!bare_machine)
      {
	//matth: don't do this for COOL.  We use Main.main instead.
	//make_label_global ("main");   /* COOL */
	//record_label ("main", 0, 0);  /* COOL */
      }
    }
  initialize_scanner (stdin);
  delete_all_breakpoints ();
  statistics_reset(&global_stats);    /* Added for COOL */
}
...
#endif
{
  ...
  if (keep_stats){statistics_print(&global_stats,stdout);}/* Added for COOL */
