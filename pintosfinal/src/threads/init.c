...
main (void)
{
  ...
  exception_init ();
  syscall_init ();
  //hash_init(&Frame_Table_Hash, Frame_Table_Hash_Func, Frame_Table_Less, NULL);
...
#ifdef USERPROG
  hash_init(&Swap_Table_Hash,Swap_Table_Hash_Func,Swap_Table_Less,NULL);
  swap_disk = disk_get(1,1);
  CurrSector=0;
  lock_init (&disk_lock);
  lock_init (&frame_lock);
  lock_init (&swapTable_lock);
  lock_init (&bitmap_lock);
  
  Swap_Bit_Map = bitmap_create(disk_size(swap_disk)/8);
...
//while(i)
#ifdef USERPROG
  int returnValue =  process_wait (process_execute (task));
   strtok_r(task, " " , &saveptr); 
 // printf("%s: exit(%d)\n",task,returnValue);
#else
  run_test (task);
#endif
