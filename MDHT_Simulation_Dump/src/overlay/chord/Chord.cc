...
void Chord::changeState(int toState)
{
    ...
    case INIT:
        ...
        //bootstrapList->insertBootstrapCandidate(overlay->getThisNode(), thisLevel);
    ...
    case BOOTSTRAP:
        ...
        // TODO hier ansetzen f�r das erzeugen der einzelnen Ringe?s
        // find a new bootstrap node and enroll to the bootstrap list
        bootstrapNode = bootstrapList->getBootstrapNode(thisLevel, thisSegment);

        //std::cout << "BS: " << thisNode << " " << thisLevel << "/" << thisSegment << " with " << bootstrapNode << endl;

        // Fix die Ports auf die einzelnen Ringe setzen
        bootstrapNode.setPort(thisNode.getPort());

    	EV << "[Chord::changeState() @ " << thisNode.getAddress()
    	<< " (" << thisNode.getKey().toString(16) << ")\n"
    	<< "   Returned bootstrapnode: " << bootstrapNode.getAddress() << ":"
    	<< bootstrapNode.getPort() << endl;
    ...
    case READY:
        ...
        bootstrapList->insertBootstrapCandidate(overlay->getThisNode(), thisLevel, thisSegment);
...
void Chord::handleJoinTimerExpired(cMessage* msg)
{
    ...
    EV << "Chord::handleJoinTimerExpired(): level is " << thisLevel << endl;
