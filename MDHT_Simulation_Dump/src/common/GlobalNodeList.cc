...
const NodeHandle& GlobalNodeList::getBootstrapNode(const NodeHandle &node)
{
    ...
    if (!node.isUnspecified()) {
        ...
        // get random node with same node type, if found return this as the bootstrapNode
                                                ...
                                                bool inoffensiveNeeded)
{
    ...
    if (inoffensiveNeeded &&
        ...
        /** solange
          * it(peerSet) am ende ist (also nix gefunden wurde)
          * nodeType nicht der geforderte ist
          * die Node nicht fertig bootstrapped ist
          * irgendwas anderes ;)
          */
