...
int MDHTBaseApp::numInitStages() const
{
    return MAX_STAGE_APP + 1;
}

void MDHTBaseApp::initialize(int stage)
{
...
void MDHTBaseApp::initializeApp(int stage)
{
    // ...
}

// Process messages passed up from the overlay.
void MDHTBaseApp::handleMessage(cMessage* msg)
{
    ...
    if (msg->arrivedOn("from_lowerTier") ||
        ...
        if (debugOutput && !ev.isDisabled()) {
            ...
            EV << "[MDHTBaseApp:handleMessage() @ " << thisNode.getAddress()
...
CompType MDHTBaseApp::getThisCompType()
{
    ...
    if (parentName == std::string("tier1")) {
        ...
        throw cRuntimeError("MDHTBaseApp::getThisCompType(): "
                             "Unknown module type!");
    }
...
void MDHTBaseApp::receiveChangeNotification(int category, const cPolymorphic * details)
{
    Enter_Method_Silent();
...
void MDHTBaseApp::handleTransportAddressChangedNotification()
{
    // ...
}

void MDHTBaseApp::handleNodeLeaveNotification()
{
    // ...
}

void MDHTBaseApp::handleNodeGracefulLeaveNotification()
{
    // ...
}

void MDHTBaseApp::callRoute(const OverlayKey& key, cPacket* msg,
                        ...
                        RoutingType routingType)
{
    ...
    // debug message
    if (debugOutput && !ev.isDisabled()) {
        EV << "[MDHTBaseApp::callRoute() @ " << thisNode.getAddress()
        << " (" << overlay->getThisNode().getKey().toString(16) << ")]\n"
        << "    Sending " << *msg
...
void MDHTBaseApp::deliver(OverlayKey& key, cMessage* msg)
{
    // deliver...
...
void MDHTBaseApp::forward(OverlayKey* key, cPacket** msg, NodeHandle* nextHopNode)
{
    // usually do nothing
}

void MDHTBaseApp::forwardResponse(const OverlayKey& key, cPacket* msg,
...
void MDHTBaseApp::update(const NodeHandle& node, bool joined)
{
}

void MDHTBaseApp::handleCommonAPIMessage(CommonAPIMessage* commonAPIMsg)
{
        ...
        case KBR_DELIVER:
        {
            ...
            if(tempMsg != NULL) {
                ...
                if ((!key.isUnspecified() && key != apiMsg->getDestKey()) ||
                    ...
                    // debug message
                    if (debugOutput) {
                        EV << "[MDHTBaseApp:handleCommonAPIMessage() @ "
                        << thisNode.getAddress() << " ("
                        << overlay->getThisNode().getKey().toString(16) << ")]\n"
...
void MDHTBaseApp::handleUpperMessage(cMessage* msg)
{
    delete msg;
}

void MDHTBaseApp::handleLowerMessage(cMessage* msg)
{
    delete msg;
}

void MDHTBaseApp::handleUDPMessage(cMessage *msg)
{
    delete msg;
}

void MDHTBaseApp::handleReadyMessage(CompReadyMessage* msg)
{
    delete msg;
}

void MDHTBaseApp::handleTraceMessage(cMessage* msg)
{
...
void MDHTBaseApp::sendMessageToLowerTier(cPacket* msg)
{
    RECORD_STATS(numOverlaySent++; bytesOverlaySent += msg->getByteLength());
...
void MDHTBaseApp::finish()
{
    // record scalar data
    simtime_t time = globalStatistics->calcMeasuredLifetime(creationTime);

    string baseAppName = string("MDHTBaseApp (") += string(this->getName())
                         += string("): ");
...
void MDHTBaseApp::finishApp()
{
    // ...
}

void MDHTBaseApp::bindToPort(int port)
{
    EV << "[MDHTBaseApp::bindToPort() @ " << thisNode.getAddress()
...
void MDHTBaseApp::sendMessageToUDP(const TransportAddress& destAddr, cPacket *msg)
{
    ...
    // debug message
    if (debugOutput) {
        EV << "[MDHTBaseApp::sendMessageToUDP() @ " << thisNode.getAddress()
        << " (" << overlay->getThisNode().getKey().toString(16) << ")]\n"
...
//private
bool MDHTBaseApp::internalHandleRpcCall(BaseCallMessage* msg)
{
    // if RPC was handled return true, else tell the parent class to handle it
    return BaseRpc::internalHandleRpcCall(msg);
}

void MDHTBaseApp::internalHandleRpcResponse(BaseResponseMessage* msg,
...
void MDHTBaseApp::internalSendRouteRpc(BaseRpcMessage* message,
...
void MDHTBaseApp::internalSendRpcResponse(BaseCallMessage* call,
