...
std::ostream& operator<<(std::ostream& Stream, const DhtDataEntry entry)
{
    return Stream << "Value: " << entry.value
                  << " Kind: " << entry.kind
                  << " ID: " << entry.id
                  << " Endtime: " << (entry.ttlMessage ? entry.ttlMessage->getArrivalTime() : 0)
...
{
	EV << "[DHTDataStorage::getDataEntry()] "  << this->getFullName()
	   << "   get data for key " << key << endl;
                             ...
                             bool responsible)
{
    ...
    entry.endTime = (ttlMessage ? ttlMessage->getArrivalTime().dbl() : 0);
    ...
    EV << "[DHTDataStorage::addData()] insert key " << key << endl;
                                ...
                                uint32_t id)
{
    ...
    while (pos.first != pos.second) {
        ...
        if (((kind == 0) || (pos.first->second.kind == kind)) &&
            ...
            if (pos.first->second.ttlMessage)
...
DhtDumpVector* DHTDataStorage::dumpDht(const OverlayKey& key, uint32_t kind,
                                       uint32_t id)
{
	EV << "[DHTDataStorage::dumpDht()] method called for key " << key << endl;
    ...
    for (; iter != end; iter++) {
        if (((kind == 0) || (iter->second.kind == kind)) &&
            ...
            entry.setTtl((iter->second.ttlMessage ? (int)SIMTIME_DBL(
                        iter->second.ttlMessage->getArrivalTime() - simTime()) : 0));
            ...
            entry.setEndTime(iter->second.endTime);
...
void DHTDataStorage::display()
{
    ...
    for (DhtDataMap::iterator it = dataMap.begin();
        ...
        cout << "Key: " << it->first << " Kind: " << it->second.kind
             ...
             << (it->second.ttlMessage ? it->second.ttlMessage->getArrivalTime() : 0) << endl;
    }
}
