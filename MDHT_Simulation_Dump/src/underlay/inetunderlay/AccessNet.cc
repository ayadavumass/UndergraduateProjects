...
void AccessNet::initialize(int stage)
{
    ...
    lastIP = 0;
     ...
     {
      hostnameInfo = dynamic_cast<HostnameInfo*>(simulation.getModuleByPath("globalObserver.globalFunctions[1].function"));

      cGate* g = router.module->gate("pppg$i", 0);
      cGate* g2 = g->getPathStartGate();
      cModule* backboneRouter = g2->getOwnerModule()->getParentModule()->getParentModule();
      RoutingTable* rt = static_cast<RoutingTable*> (backboneRouter->getSubmodule("routingTable"));

      IPvXAddress backboneRouterAddress(rt->getRouterId());
      IPvXAddress routerAddress(IPAddress(router.IPAddress));
      hostnameInfo->addNode(IPvXAddress(), backboneRouterAddress, false);
      hostnameInfo->addNode(backboneRouterAddress, routerAddress, false);
    }
...
int AccessNet::addOverlayNode(cModule* node, bool migrate)
{
    ...
    {
      IPvXAddress routerAddress(IPAddress(router.IPAddress));
      IPvXAddress terminalAddress(IPAddress(terminal.IPAddress));
      
      hostnameInfo->addNode(routerAddress, terminalAddress, true);
    }
